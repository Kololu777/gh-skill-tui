...
| `P` | outside skill の source 内追加先を手動で指定して PR / MR の計画を作る |
...
source 外の skill（`O`）を取り込みたい場合も `p` を使えます。追加先を自動判定できないときは `P` でディレクトリと skill 名を指定します。
...
# outside skill を p で追加する際、追加先を推定できない場合の既定先
# 未設定なら source 内の share/ を使う
